...
import './DessertCard.css'
import { DessertCardProps } from '../ts/types'
...
export default function DessertCard({
  ...
  const handleQuantityMinus = () => cartQuantityMinus()

  const handleQuantityPlus = () => cartQuantityPlus()

  return (
    <article className="dessert-card">
      <picture>
        <source media="(max-width: 640px)" srcSet={srcMobile} />
        <source media="(max-width: 900px)" srcSet={srcTablet} />
        <img src={srcDesktop} alt="Waffles with berries" />
      </picture>

      <div className={`add-to-cart-wrapper ${isAdded ? 'added' : ''}`}>
        {isAdded && isAdded
          ? (
            <>
              <button
      ...
      <div className="dessert-card-text flow">
        <p>{category}</p>
        <h3>{name}</h3>
        <span className="dessert-pricing">${price.toFixed(2)}</span>
      </div>
    </article>
  )
}